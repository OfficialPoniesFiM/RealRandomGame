...
waitTillEnd = 4000 #This is how much time the game has until exiting if the enemy approaches the main character. This variable lowers itself every refresh. The game refreshes 4000 times a second, so 4000 ticks = 4 seconds.
...
while whiles:
    ...
    if directionToGoFirst == 1: #Mostly useless.
        if hitSniper == False: #This checks if the enemy Sniper isn't hit.
            otherX1 += .15 * currentFrameTime #The enemy sniper goes to the left by adding .15 to its X position * milliseconds from last frame. Only happens when not hit.
        
        if hitCreeper == False: #This checks if the Creeper isn't hit.
            otherX2 -= .15 * currentFrameTime #The Creeper goes to the right by removing .15 from its X position * milliseconds from last frame. Only happens when not hit.
    ...
    if characterXLeft == characterXRight: #If the player isn't pressing the left(or A), or right(or D) keys, or pressing both at the same time.
        ...
        characterX -= .15 * currentFrameTime #.15 * milliseconds from last frame is removed from its X position to move left.
